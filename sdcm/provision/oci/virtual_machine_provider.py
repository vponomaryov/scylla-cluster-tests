...
class VirtualMachineProvider:
    ...
    def __post_init__(self):
        self._compute_client = self._oci_service.get_compute_client(self._region)
    ...
    def _provision_instance(
        ...
        instance = None
        LOGGER.info("Going to create an instance with the following launch params: %s", launch_details)
        try:
    ...
    def attach_secondary_vnics(
        ...
        for nic_index in range(1, nic_count):
            if nic_index < existing_count:
                LOGGER.debug("Secondary VNIC at nic_index=%d already attached to '%s'", nic_index, name_or_id)
                continue
