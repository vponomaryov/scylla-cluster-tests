...
class OciRegion:
    ...
    SCT_SUBNET_NAME_TMPL = "SCT-2-subnet-regional{suffix}"
    ...
    def _create_private_route_table(self):
        ...
        response = self.network_client.create_route_table(details)
        return self._wait_for_state(
            self.network_client,
            self.network_client.get_route_table,
            response.data.id,
            "AVAILABLE",
        )
    ...
    def _create_security_list(self):
        ...
        return self._wait_for_state(
            self.network_client,
            self.network_client.get_security_list,
            response.data.id,
            "AVAILABLE",
        )
    ...
    def _create_nsg(self, display_name, role):
        ...
        nsg = self.network_client.create_network_security_group(details).data
        nsg = self._wait_for_state(
            self.network_client,
            self.network_client.get_network_security_group,
            nsg.id,
            "AVAILABLE",
        )
        ...
        ports_to_open = set()
        if role == "db":
            ports_to_open.update([enum_element.value for enum_element in list(ScyllaOpenPorts)])
        elif role == "monitor":
        ...
        LOGGER.info(
            "Going to open following ports in the '%s' NSG: %s",
            display_name,
            ", ".join(map(str, ports_to_open)),
        )
        ipv4_rules_details, ipv6_rules_details = [], []
        ...
        for security_rules in (ipv4_rules_details, ipv6_rules_details):
            if security_rules:
    ...
    def _create_internet_gateway(self):
        ...
        igw = self._wait_for_state(
            self.network_client,
            self.network_client.get_internet_gateway,
            response.data.id,
            "AVAILABLE",
        )
    ...
    def _find_nat_gateway(self):
        ...
        nat_gateways = oci.pagination.list_call_get_all_results_generator(
            self.network_client.list_nat_gateways,
            yield_mode="record",
            compartment_id=self.compartment_id,
            vcn_id=self.vcn.id,
        )
    ...
    def _create_nat_gateway(self):
        ...
        response = self.network_client.create_nat_gateway(details)
        ngw = self._wait_for_state(
            self.network_client,
            self.network_client.get_nat_gateway,
            response.data.id,
            "AVAILABLE",
        )
        return ngw
    ...
    def _get_home_region(self) -> str:
        ...
        try:
            region_subscriptions = oci.pagination.list_call_get_all_results_generator(
                self.identity_client.list_region_subscriptions, yield_mode="record", tenancy_id=self._config["tenancy"]
            )
            first_region = ""
